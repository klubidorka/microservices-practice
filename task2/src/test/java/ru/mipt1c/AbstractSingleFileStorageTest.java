...
public abstract class AbstractSingleFileStorageTest extends KeyValueStorageFactories {
    ...
    public void testPersistence() {
        ...
        StorageTestUtils.doInTempDirectory(path -> doWithPojo(path, storage -> assertNotNull(storage.read(KEY_1))));
    }
    ...
    public void testNonEquality() {
        StorageTestUtils.doInTempDirectory(path -> assertSame(doWithPojo(path, null), doWithPojo(path, null)));
        StorageTestUtils.doInTempDirectory(path -> assertSame(doWithStrings(path, null), doWithStrings(path, null)));
        StorageTestUtils.doInTempDirectory(path -> assertSame(doWithNumbers(path, null), doWithNumbers(path, null)));
    }
